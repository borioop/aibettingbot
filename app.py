...
# === CACHE FILES ===
FILTERED_FIXTURES_FILE = os.path.join(CACHE_DIR, 'filtered_fixtures.json')
PREDICTIONS_CACHE_FILE = os.path.join(CACHE_DIR, 'predictions.json')
ODDS_CACHE_FILE = os.path.join(CACHE_DIR, 'odds.json')
...
# === DISK CACHE HELPERS ===
def load_cache_from_disk(filepath):
    """Load cache from disk"""
    try:
        if os.path.exists(filepath):
            with open(filepath, 'r', encoding='utf-8') as f:
                data = json.load(f)
                app.logger.info(f"✅ Loaded cache from {filepath}")
                return data
    except Exception as e:
        app.logger.error(f"❌ Error loading cache from {filepath}: {e}")
    return {}
...
def save_cache_to_disk(filepath, data):
    ...
    try:
        ...
        app.logger.info(f"💾 Saved cache to {filepath}")
    except Exception as e:
        app.logger.error(f"❌ Error saving cache to {filepath}: {e}")

# === LOAD CACHES FROM DISK ON STARTUP ===
filtered_fixtures_cache = load_cache_from_disk(FILTERED_FIXTURES_FILE)
predictions_cache = load_cache_from_disk(PREDICTIONS_CACHE_FILE)
odds_cache = load_cache_from_disk(ODDS_CACHE_FILE)

app.logger.info(f"📊 Loaded {len(filtered_fixtures_cache)} filtered fixtures from disk")
app.logger.info(f"📊 Loaded {len(predictions_cache)} predictions from disk")
app.logger.info(f"📊 Loaded {len(odds_cache)} odds from disk")
...
def update_fixtures_cache():
    """Update cache for yesterday + next 3 days - SAVES TO DISK"""
    while True:
        try:
            ...
            app.logger.info("💾 Saving all caches to disk...")
            save_cache_to_disk(FILTERED_FIXTURES_FILE, filtered_fixtures_cache)
            save_cache_to_disk(PREDICTIONS_CACHE_FILE, predictions_cache)
            save_cache_to_disk(ODDS_CACHE_FILE, odds_cache)
            app.logger.info("✅ All caches saved to disk")

            app.logger.info(f"✅ Cache update completed. Sleeping for {CACHE_DURATION} seconds...")
...
def initial_cache_population():
    """Initial cache population on startup - SEQUENTIAL"""
    ...
    # SAVE TO DISK
    app.logger.info("💾 Saving initial caches to disk...")
    save_cache_to_disk(FILTERED_FIXTURES_FILE, filtered_fixtures_cache)
    save_cache_to_disk(PREDICTIONS_CACHE_FILE, predictions_cache)
    save_cache_to_disk(ODDS_CACHE_FILE, odds_cache)
...
def api_fixtures():
    """Get UPCOMING/LIVE fixtures by date - FROM CACHE"""
...
def process_finished_fixture(fixture):
    """Process a single FINISHED fixture"""
    try:
        fixture_id = fixture.get("fixture", {}).get("id")
        if not fixture_id:
            return None
...
if __name__ == "__main__":
    app.logger.info("=" * 80)
    app.logger.info("🚀 FOOTBALL APP STARTING WITH DISK CACHE")
    app.logger.info("=" * 80)

    # Check if cache exists on disk AND has data
    cache_has_data = False
    if os.path.exists(FILTERED_FIXTURES_FILE) and filtered_fixtures_cache:
        # Check if cache has recent data (not older than 3 hours)
        ...
        if recent_data_found:
            app.logger.info("✅ CACHE LOADED FROM DISK - App ready immediately!")
            app.logger.info(f"📊 Loaded cache for {len(filtered_fixtures_cache)} dates")
            cache_has_data = True
            cache_ready.set()
    
    if not cache_has_data:
        app.logger.info("⏳ No valid cache found, starting initial population...")
        app.logger.info("⚠️  This will take 5-10 minutes on first run...")
        initial_cache_population()
        app.logger.info("✅ Initial population completed!")
    ...
    app.logger.info("💾 Cache stored on disk in: " + CACHE_DIR)
